...
const MobileMenu: FC<MobileMenuProps> = ({ setMobMenu, customStyle }) => {
	return (
		<nav
			className={`md:hidden flex transition-all flex-col gap-y-5 bg-maroon fixed shadow-xl left-0 top-0 bottom-0 py-11 px-5 gap-x-6 text-yellow ${customStyle}`}
		>
			<button
			...
			{mobileMenu.map((item, index) => (
				<Link
					...
					spy={true}
					smooth={true}
					className='font-normal text-xl leading-6 uppercase cursor-pointer'
					duration={500}
					offset={
